...
for host in "${hosts[@]}"; do
  ...
  grep -Eq "host_id.*$host|Host ID.*$host" "docs/hosts/$host.md"
done
...
test "$(grep -Ec 'mcp\.AddTool\(' internal/mcpserver/server.go)" = "3"
grep -q '"command": "agent-runtime-proof"' plugin/agent-runtime-proof/.mcp.json
grep -q '"args": \["mcp"\]' plugin/agent-runtime-proof/.mcp.json
if grep -ERn '"command"[[:space:]]*:[[:space:]]*"(sh|bash|zsh|fish|cmd|powershell|pwsh)(\.exe)?"' plugin testdata/host-configs profiles/hosts; then
...
if grep -ERn '/Users/fanhcy|[A-Za-z]:\\Users\\fanhcy|AKIA[0-9A-Z]{16}|sk-[A-Za-z0-9]{20,}' docs/hosts docs/host-configuration.md plugin testdata/host-configs profiles/hosts; then
  echo "developer path or credential-shaped text found" >&2
  exit 1
fi
if grep -Eq 'Set-Content[[:space:]]+-Encoding[[:space:]]+utf8' scripts/run_windows_phase4_acceptance.ps1; then
