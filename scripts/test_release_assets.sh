...
for relative in scripts/build_release_assets.sh scripts/package_release_asset.py scripts/verify_release_assets.sh cmd/agent-runtime-proof/main.go internal/versioninfo; do
...
git -C "$test_root/repo" add -- scripts cmd internal/versioninfo
...
test -n "$output"
printf '%s\n' '{"bomFormat":"CycloneDX","specVersion":"1.6","metadata":{"component":{"name":"github.com/fantasyce/agent-runtime-proof","version":"v1.0.0"}}}' > "$output"
FAKE
...
bash scripts/verify_release_assets.sh "$test_root/dist" 1.0.0 "$(git rev-parse HEAD)"
CYCLONEDX_GOMOD="$test_root/bin/cyclonedx-gomod" \
...
expected=(
  SHA256SUMS
  agent-runtime-proof_1.0.0_source.tar.gz
  agent-runtime-proof_1.0.0_darwin_arm64.tar.gz
  agent-runtime-proof_1.0.0_darwin_arm64.cdx.json
  agent-runtime-proof_1.0.0_linux_amd64.tar.gz
  agent-runtime-proof_1.0.0_linux_amd64.cdx.json
  agent-runtime-proof_1.0.0_windows_amd64.zip
  agent-runtime-proof_1.0.0_windows_amd64.cdx.json
)
