...
go build -trimpath -o "$payload_dir/bin/fixture-runtime" "$repo_dir/testdata/acceptance/helper"
...
[[ "$leaked_exit" -eq 2 ]] && jq -e '.verdict == "LEAKED" and (.reason_codes | index("RUNTIME_OUTSIDE_ALLOWED_ROOT"))' "$run_dir/leaked.json" >/dev/null
[[ "$not_running_exit" -eq 2 ]] && jq -e '.verdict == "NOT_RUNNING" and .observation.process == null' "$run_dir/not-running.json" >/dev/null
[[ "$limit_exit" -eq 3 ]] && jq -e '.verdict == "UNKNOWN" and (.reason_codes | index("ARTIFACT_SCAN_LIMIT_EXCEEDED"))' "$run_dir/limit.json" >/dev/null
[[ "$tree_exit" -eq 3 ]] && jq -e '.verdict == "UNKNOWN" and (.reason_codes | index("PLATFORM_EVIDENCE_UNAVAILABLE"))' "$run_dir/tree.json" >/dev/null
...
if rg -n 'token-secret|/Users/|process\.argv[^\"]*:' \
  ...
  "$run_dir/tree.json" "$run_dir/inspect.json" "$run_dir/doctor.json"; then
